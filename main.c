...
void PWR_initfile() {
    pwr_f = fopen("devices.json", "w");
    fclose(pwr_f);
}

void PWR_writedevice(const char *name, const char *address) {
    pwr_f = fopen("devices.json", "a");
    fprintf(pwr_f,"Device: %s %s", name, address);
    fclose(pwr_f);
}

void PWR_writeuuid(const char *uuid) {
    pwr_f = fopen("devices.json", "a");
    fprintf(pwr_f," UUID: %s\n", uuid);  
    fclose(pwr_f);
}

void PWR_write(const char *name, const char *address, const char *uuid) {
    pwr_f = fopen("devices.json", "a");
    
    struct stat st;
    stat("devices.json", &st);
    int size;
    size = st.st_size;
    ...
    fprintf(pwr_f,"{'name' : '%s', 'address' : '%s', 'uuid' : '%s'}", name, address, uuid);  
    fclose(pwr_f);
}



/*End of PWR Team Edit Area*/
...
static void update_device_info(GDBusProxy *proxy)
{
	...
	if (parse_service_data(proxy, MESH_PROV_SVC_UUID, &prov_data)) {
        ...
        struct prov_svc_data *prov_data2 = &prov_data;

        for (i = 0; i < 16; ++i) {
            sprintf(txt_uuid + (i * 2), "%2.2x", prov_data2->dev_uuid[i]);
        }
        ...
        PWR_write(name, address, txt_uuid);
...
static void set_discovery_filter_reply(DBusMessage *message, void *user_data)
{
	...
	bt_shell_printf("SetDiscoveryFilter seb2 success\n");
